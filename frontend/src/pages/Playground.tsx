...
export default function Playground() {
  ...
  const handleShowStats = async () => {
      if (user?.id) {
          ...
          // Set default stats if user is not logged in
          setStats({
              total_cost: 0,
              total_input_tokens: 0,
              total_output_tokens: 0,
  ...
  const renderParameters = () => {
    if (mode === 'text') {
        return (
            <div className="mt-4 p-4 bg-gray-700/50 rounded border border-gray-700 flex gap-6 text-sm flex-wrap items-end">
                <div className="flex flex-col gap-1 w-[200px]">
                    <div className="flex justify-between">
                        <label className="text-gray-400 font-bold text-xs uppercase">{t('common.temp')}</label>
                        <span className="text-xs text-gray-300 font-mono">{textParams.temperature}</span>
                    </div>
                    <input 
                ...
                <div className="flex flex-col gap-1 w-[200px]">
                    <div className="flex justify-between">
                        <label className="text-gray-400 font-bold text-xs uppercase">{t('common.top_p')}</label>
                        <span className="text-xs text-gray-300 font-mono">{textParams.top_p}</span>
                    </div>
                    <input 
                ...
                <div className="flex flex-col gap-1">
                    <label className="text-gray-400 font-bold text-xs uppercase mb-1">{t('common.search_web')}</label>
                    <button 
        ...
        return (
            <div className="mt-4 p-4 bg-gray-700/50 rounded border border-gray-700 flex gap-6 text-sm flex-wrap">
                <div className="flex flex-col gap-1">
                    <label className="text-gray-400 font-bold text-xs uppercase">{t('common.resolution')}</label>
                    <select 
                      value={videoParams.size} 
                      onChange={e => setVideoParams({...videoParams, size: e.target.value})}
                      className="border border-gray-700 rounded p-1.5 bg-gray-800/60 text-white min-w-[150px]"
                    >
                ...
                <div className="flex flex-col gap-1">
                    <label className="text-gray-400 font-bold text-xs uppercase">{t('common.duration')}</label>
                    <select 
                 ...
                 <div className="flex flex-col gap-1">
                    <label className="text-gray-400 font-bold text-xs uppercase">{t('common.prompt_extend')}</label>
                    <select 
                      value={String(videoParams.prompt_extend)} 
                      onChange={e => setVideoParams({...videoParams, prompt_extend: e.target.value === 'true'})}
                      className="border border-gray-700 rounded p-1.5 bg-gray-800/60 text-white min-w-[100px]"
                    >
        ...
        return (
            <div className="mt-4 p-4 bg-gray-700/50 rounded border border-gray-700 flex gap-6 text-sm flex-wrap">
                <div className="flex flex-col gap-1">
                 ...
                 <div className="flex flex-col gap-1">
                    <label className="text-gray-400 font-bold text-xs uppercase">{t('common.count')}</label>
                    <select 
                      value={imageParams.n} 
                      onChange={e => setImageParams({...imageParams, n: parseInt(e.target.value)})}
                      className="border border-gray-700 rounded p-1.5 bg-gray-800/60 text-white min-w-[80px]"
                    >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 relative overflow-hidden">
      ...
      <div className="w-full max-w-[1800px] mx-auto p-6 relative z-10">
        <div className="mb-6 flex justify-between items-end">
            <div>
                <BackButton to="/dashboard" label={t('common.dashboard')} />
                <h1 className="text-3xl font-bold mt-2 text-white">{t('playground.title')}</h1>
            </div>
            
            <div className="flex gap-4 items-center">
                <button
                    onClick={handleShowStats}
                    className="px-4 py-2 rounded-md text-sm font-bold flex items-center gap-2 transition bg-gray-800/60 border border-gray-700 hover:bg-gray-700/60 text-white"
                >
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-md text-sm font-bold flex items-center gap-2 transition border ${showHistory ? 'bg-gray-700/60 border-gray-600 text-white' : 'bg-gray-800/60 border-gray-700 hover:bg-gray-700/60 text-white'}`}
                >
                    <span>🕒</span> {t('common.history')}
                </button>

                <div className="flex bg-gray-800/60 rounded-lg p-1 shadow-sm border border-gray-700">
                    {MODES.map(m => (
                        <button
                            key={m.id}
                            onClick={() => setMode(m.id)}
                            className={`px-4 py-2 rounded-md text-sm font-bold flex items-center gap-2 transition ${mode === m.id ? 'bg-blue-900/50 text-blue-300' : 'text-gray-300 hover:bg-gray-700/60'}`}
                        >
        ...
        <div className="grid gap-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="rounded-lg bg-gray-800/60 p-6 shadow-lg border border-gray-700">
                <h2 className="mb-4 text-sm font-bold text-gray-300 uppercase tracking-wide">{t('playground.load_library')}</h2>
                <div className="space-y-3">
                    <select 
                        className="w-full rounded border border-gray-700 p-2 text-sm bg-gray-700/50 text-white"
                        value={selectedTag}
                    ...
                    <div className="pt-2 border-t border-gray-700">
                        <label className="text-xs font-bold text-gray-400 uppercase mb-1 block">Knowledge Base</label>
                        <select 
                            className="w-full rounded border border-gray-700 p-2 text-sm bg-gray-700/50 text-white"
                            value={selectedKbId}
            ...
            <div className="rounded-lg bg-gray-800/60 p-6 shadow-lg border border-gray-700 flex flex-col h-[500px]">
              <div className="flex justify-between items-center mb-4">
                  <h2 className="text-sm font-bold text-gray-300 uppercase tracking-wide">{t('editor.content_label')}</h2>
                  <div className="flex gap-2">
                    <button
                        onClick={() => navigate(`/optimizer?content=${encodeURIComponent(prompt)}${selectedPromptId ? `&promptId=${selectedPromptId}` : ''}`)}
                        className="text-xs bg-purple-900/30 text-purple-300 px-2 py-1 rounded hover:bg-purple-900/50 flex items-center gap-1 disabled:opacity-50"
                         disabled={!prompt}
                     >
                    ...
                    {selectedPromptId && (
                        <button 
                            ...
                            disabled={isUpdating}
                            className="text-xs bg-blue-900/30 text-blue-300 px-2 py-1 rounded hover:bg-blue-900/50 disabled:opacity-50"
                        >
              ...
              <div className="border border-gray-700 rounded overflow-hidden" style={{ height: '400px' }}>
                <MarkdownEditor
                  ...
                  visible={true}
                  enableScroll={true}
                  style={{ height: '100%', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace', color: '#e2e8f0' }}
                />
              </div>
            </div>
            ...
            <div className="rounded-lg bg-gray-800/60 p-6 shadow-lg border border-gray-700">
               <div className="flex items-center justify-between mb-4">
                 <h2 className="text-sm font-bold text-gray-300 uppercase tracking-wide">{t('playground.variables')}</h2>
                 <button onClick={addVariable} className="text-xs text-blue-400 font-bold hover:text-blue-300">+ ADD</button>
               </div>
               <div className="space-y-2 max-h-[200px] overflow-y-auto">
                   {variables.map((v, i) => (
                     <div key={i} className="flex gap-2">
                       <input
                         className="w-1/3 rounded border border-gray-700 p-2 text-xs bg-gray-700/50 text-white"
                         placeholder="Key"
                         value={v.key}
                         onChange={(e) => updateVariable(i, 'key', e.target.value)}
                       />
                       <input
                         className="flex-1 rounded border border-gray-700 p-2 text-xs bg-gray-700/50 text-white"
                         placeholder="Value"
                         value={v.value}
                         onChange={(e) => updateVariable(i, 'value', e.target.value)}
                       />
                       <button onClick={() => removeVariable(i)} className="text-gray-400 hover:text-red-400 px-1">&times;</button>
                     </div>
                   ))}
               </div>
            </div>
          ...
          <div className="lg:col-span-8 flex flex-col h-full space-y-6">
             ...
             {/* Model & Settings */}
             <div className="bg-gray-800/60 p-4 rounded-lg shadow-lg border border-gray-700">
                 <div className="flex items-center justify-between">
                     <div className="flex gap-4 items-center">
                         <span className="text-sm font-bold text-gray-300 uppercase">{t('common.models')}:</span>
                         <div className="flex gap-2">
                     ...
                     <div className="flex flex-col items-end mr-4">
                         ...
                         <span className="text-sm font-mono text-gray-300">~¥{getEstimatedCost()}</span>
                     </div>
             ...
             <div className={`grid gap-4 flex-1 ${selectedModels.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                 {selectedModels.map(modelId => {
                     ...
                     return (
                         <div key={modelId} className="bg-gray-800/60 rounded-lg shadow-lg border border-gray-700 flex flex-col overflow-hidden h-[600px]">
                             <div className="bg-gray-700/50 px-4 py-2 border-b border-gray-700 flex justify-between items-center">
                                 <span className="font-bold text-gray-300 text-sm">{modelName}</span>
                                 {result?.output && (
                                     <button 
                             ...
                             <div className="flex-1 p-4 overflow-auto bg-gray-800/40">
                                 {result?.loading ? (
                                     ...
                                     mode === 'image' ? (
                                         <div className="flex items-center justify-center h-full bg-black/10 rounded">
                                             <img src={result.output} alt="Generated" className="max-w-full max-h-full object-contain rounded shadow-sm" />
                                         </div>
                                     ) : mode === 'video' ? (
                                         <div className="flex flex-col items-center justify-center h-full bg-black/10 rounded p-4">
                                             <video 
                                                src={result.output} 
                                                controls 
                                             ...
                                             <a href={result.output} target="_blank" rel="noreferrer" className="text-xs text-blue-400 hover:underline">
                                                 {t('common.open_video')}
                                             </a>
                                         </div>
                                     ) : (
      ...
      {/* History Drawer */}
      {showHistory && (
          <div className="fixed inset-y-0 right-0 w-96 bg-gray-800/80 shadow-2xl z-50 flex flex-col border-l border-gray-700 animate-slide-in-right">
              <div className="p-4 border-b border-gray-700 flex justify-between items-center bg-gray-700/50">
                  <h2 className="font-bold text-white">{t('common.execution_history')}</h2>
                  ...
                  {history.length === 0 ? (
                      ...
                      history.map(item => (
                          <div 
                              ...
                              className="bg-gray-700/50 border border-gray-700 rounded-lg p-3 hover:border-blue-600/50 hover:shadow-md transition cursor-pointer group"
                          >
                              <div className="flex justify-between items-start mb-2">
                                  <span className={`text-xs font-bold px-2 py-0.5 rounded ${item.modelType === 'video' ? 'bg-purple-900/50 text-purple-300' : item.modelType === 'image' ? 'bg-pink-900/50 text-pink-300' : 'bg-blue-900/50 text-blue-300'}`}>
                                      {item.modelType || 'TEXT'}
                                  </span>
                              ...
                              <div className="text-xs font-semibold text-gray-300 mb-1">{item.modelName}</div>
                              <div className="text-xs text-gray-400 line-clamp-2 mb-2 font-mono bg-gray-700/50 p-1 rounded">
                                  {item.prompt}
                              </div>
                              ...
                              {item.modelType === 'video' && item.result && (
                                  <div className="w-full h-24 bg-black/30 rounded flex items-center justify-center text-white text-xs">Video Result</div>
                              )}
                          </div>
                      ))
                  )}
      ...
      {showStats && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
              <div className="bg-gray-800/80 rounded-xl shadow-2xl p-6 w-full max-w-md animate-scale-in border border-gray-700">
                  <div className="flex justify-between items-center mb-6">
                      <h2 className="text-xl font-bold text-white">{t('common.usage_stats')}</h2>
                      <button onClick={() => setShowStats(false)} className="text-gray-400 hover:text-white text-xl">&times;</button>
                  </div>
                  
                  <div className="space-y-4">
                      <div className="bg-blue-900/30 p-4 rounded-lg flex justify-between items-center border border-blue-800/50">
                          <span className="text-blue-300 font-bold">{t('common.total_cost')}</span>
                          <span className="text-2xl font-bold text-blue-300">¥ {Number(stats.total_cost).toFixed(4)}</span>
                      </div>
                      
                      <div className="grid grid-cols-2 gap-4">
                          <div className="bg-gray-700/50 p-3 rounded border border-gray-700">
                              <div className="text-xs text-gray-400 uppercase">{t('common.input_tokens')}</div>
                              <div className="text-lg font-mono font-semibold text-white">{stats.total_input_tokens}</div>
                          </div>
                          <div className="bg-gray-700/50 p-3 rounded border border-gray-700">
                              <div className="text-xs text-gray-400 uppercase">{t('common.output_tokens')}</div>
                              <div className="text-lg font-mono font-semibold text-white">{stats.total_output_tokens}</div>
                          </div>
                          <div className="bg-gray-700/50 p-3 rounded border border-gray-700">
                              <div className="text-xs text-gray-400 uppercase">{t('common.images_gen')}</div>
                              <div className="text-lg font-mono font-semibold text-white">{stats.total_images}</div>
                          </div>
                          <div className="bg-gray-700/50 p-3 rounded border border-gray-700">
                              <div className="text-xs text-gray-400 uppercase">{t('common.videos_gen')}</div>
                              <div className="text-lg font-mono font-semibold text-white">{stats.total_videos}</div>
                          </div>
                      </div>
                  </div>
